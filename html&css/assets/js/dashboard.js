...
const currentMonthTextFields =
    document.getElementsByClassName('current-month-text');
...
function changePeriodText(displayedPeriod) {
    ...
    Array.from(currentMonthTextFields).forEach((element) => {
...
function getSalesPeriod(period) {
    ...
    try {
        if (period === 'all') {
            salesQuery =
                db.prepare(`SELECT SUM(total_revenue) as total_revenue, SUM(total_price)
     as total_sales, COUNT(customer_name) as customer_count FROM sales`);
        } else {
            salesQuery = db.prepare(
                'SELECT SUM(total_revenue) as total_revenue, SUM(total_price) as total_sales, ' +
                    "COUNT(customer_name) as customer_count FROM sales WHERE strftime('%Y-%m-%d', `purchase_time`)>='" +
                    startDate +
                    "' AND strftime('%Y-%m-%d', `purchase_time`)<='" +
                    endDate +
                    "'",
            );
        }
...
function getGraphData(period) {
    ...
    if (period === 'today') {
        ...
        query =
            db.prepare(`SELECT SUM(total_price) as total_sales FROM sales WHERE date(purchase_time)=date('now')
      AND STRFTIME('%H', purchase_time) > @start AND STRFTIME('%H', purchase_time) <= @stop`);
        ...
        query = db.prepare(
            `SELECT SUM(total_price) as total_sales FROM sales WHERE date(purchase_time)=@day`,
        );
        ...
        query =
            db.prepare(`SELECT SUM(total_price) as total_sales FROM sales WHERE 
      date(purchase_time) >= @start AND date(purchase_time) <= @stop`);
        ...
        query =
            db.prepare(`SELECT SUM(total_price) as total_sales FROM sales WHERE STRFTIME('%m', purchase_time) = @month
      ...
      AND STRFTIME('%Y', purchase_time) = @year`);
        ...
        query = db.prepare(
            `SELECT SUM(total_price) as total_sales FROM sales WHERE STRFTIME('%Y', purchase_time)= @year`,
        );
