...
function updateLastLogin(userId) {
    ...
    try {
        const updateLoginQuery = db.prepare(
            `UPDATE auth SET last_login = datetime('now') WHERE id = ?`,
        );
...
function loginUser(emailAddress, password) {
    ...
    try {
        const emailGetQuery = db.prepare(
            'SELECT password, id, first_name, last_name, is_admin FROM auth WHERE email= ?',
        );
        ...
        if (bcrypt.compareSync(password, row.password)) {
            ...
            if (row.is_admin === 1) {
                ...
                remote
                    .getCurrentWindow()
                    .loadFile('html&css/pages/dashboard/new-sale.html');
            }
        } else {
